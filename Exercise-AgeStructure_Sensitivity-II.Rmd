---
pagetitle: 'Exercise: Structured Populations - Sensitivity II'
output: 
  html_document:
    toc: FALSE
---
...
Sensitivities give the absolute change in $\lambda$ given an absolute change in the demographic parameters $a_{ij}$. Thus, sensitivities do not take into account that survival probabilities and fecundities are generally on very different scales. To correct for this, we can compute the *elasticity matrix* $\sf E$, with entries $e_{ij}$ defined as

$$
e_{ij}=\frac{a_{ij}}{\lambda}s_{ij}.
$$
To compute the whole elasticity matrix in one step, we can use the formula

$$
\mathsf E=\frac{1}{\lambda}\mathsf L*\mathsf S.
$$
In this last equation, the $*$ between the matrix $\mathsf L$ and $\mathsf S$ does not denote *normal matrix multiplication* but *element-wise multiplication*, that is, multiplication of $l_{ij}$ with $s_{ij}$ in all positions of the matrices.

Elasticities give the *proportional change* in $\lambda$ given a proportional change in the demographic parameter $a_{ij}$. In other words, it gives by how many percentages $\lambda$ changes given a change in percentage of the matrix entry $a_{ij}$. We can compute the elasticity matrix as follows:
