...
An alternative method is *sensitivty analysis*. In this method, we compute the derivative of the dominant eigenvalue $\lambda$ with respect to the different matrix entries. In other words, we calculate the slope of $\lambda$ as a function of the different matrix entries. This is what we will do in the this exercise.

## Sensistivity analysis

Recall from the lecture that the sensitivity of the dominant eigenvalue with respect to a matrix entry $a_{ij}$ is given by
...
time.series.transpose <- matrix(0, nrow=nrow(L.transpose), ncol=length+1) # creates an empty matrix of dimension 3x(length+1)
time.series.transpose[, 1] <- N.0  # stores the initial population vector N.0 in the matrix "time.series.transpose"
for (i in 1:length){# executes the multiplication of L.N.transpose(t) as often as specified by the parameter "length"
  time.series.transpose[ , i+1] <- L.transpose %*% time.series.transpose[ ,i]
}
matplot(0:length, t(time.series.transpose), type="l", lty=1:3, col = 1, ylab = "something that is NOT density", xlab = "time", main="transposed matrix") # plots result, note that this graph has NO biological interpretation, it is only a tool to obtain the left eigenvalue
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n") # defines legend
