...
In the first exercise, we found that that the dominant eigenvalue of this matrix $\mathsf L$ equals $\lambda=0.952$, thus is population is in steep decline.
...
Let us find out on which demograhic parameter we should focus our effort. To this end, we define the following population projection matrices:

```{r}
L = matrix(c(0, 1, 5, 0.25, 0, 0, 0, 0.5, 0), nrow=3, byrow=T)
...
###
time.series <- matrix(0, nrow=nrow(L), ncol=length+1) # creates an empty matrix of dimension 3x(length+1)
time.series[, 1] <- N.0  # stores the initial population vector N.0 in the matrix "time.series"
for (i in 1:length){# executes the multiplication of L.N(t) as often as specified by the parameter "length"
  time.series[ , i+1] <- L %*% time.series[ ,i]
}
matplot(0:length, t(time.series), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="original matrix") # plots result
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n") # defines legend

### same procedure as above but now for the population projection matrix L.f2
time.series.f2 <- matrix(0, nrow=nrow(L.f2), ncol=length+1)
time.series.f2[, 1] <- N.0
for (i in 1:length){
  time.series.f2[ , i+1] <- L.f2 %*% time.series.f2[ ,i]
}
matplot(0:length, t(time.series.f2), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="f2 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

### same procedure as above but now for the population projection matrix L.f3
time.series.f3 <- matrix(0, nrow=nrow(L.f3), ncol=length+1)
time.series.f3[, 1] <- N.0
for (i in 1:length){
  time.series.f3[ , i+1] <- L.f3 %*% time.series.f3[ ,i]
}
matplot(0:length, t(time.series.f3), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="f3 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

### same procedure as above but now for the population projection matrix L.s1
time.series.s1 <- matrix(0, nrow=nrow(L.s1), ncol=length+1)
time.series.s1[, 1] <- N.0
for (i in 1:length){
  time.series.s1[ , i+1] <- L.s1 %*% time.series.s1[ ,i]
}
matplot(0:length, t(time.series.s1), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="s1 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

### same procedure as above but now for the population projection matrix L.s2
time.series.s2 <- matrix(0, nrow=nrow(L.s2), ncol=length+1)
time.series.s2[, 1] <- N.0
for (i in 1:length){
  time.series.s2[ , i+1] <- L.s2 %*% time.series.s2[ ,i]
}
matplot(0:length, t(time.series.s2), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="s2 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")
```

To compute the dominant eigenvalue for each matrix, we divide the population density at the last time step by the population density at the previous time step. The code below applies this for the population density of individuals in age class 1, but, as we have seen in the first exercise, the densities in age class 2 or 3 or the total density would give the same result.
...
time.series <- matrix(0, nrow=nrow(L), ncol=length+1) # creates an empty matrix of dimension 3x(length+1)
time.series[, 1] <- N.0  # storeshe the initial population vector N.0 in the matrix "time.series"
for (i in 1:length){# executes the multiplication of L.N(t) as often as specified by the parameter "length"
  time.series[ , i+1] <- L %*% time.series[ ,i]
}
matplot(0:length, t(time.series), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="original matrix") # plots result
...
time.series.f2.prop <- matrix(0, nrow=nrow(L.f2.prop), ncol=length+1) # same procedure as above but now for the population projection matrix L.f2
time.series.f2.prop[, 1] <- N.0
for (i in 1:length){
  time.series.f2.prop[ , i+1] <- L.f2.prop %*% time.series.f2.prop[ ,i]
}
matplot(0:length, t(time.series.f2.prop), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="f2 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

time.series.f3.prop <- matrix(0, nrow=nrow(L.f3.prop), ncol=length+1) # same procedure as above but now for the population projection matrix L.f3
time.series.f3.prop[, 1] <- N.0
for (i in 1:length){
  time.series.f3.prop[ , i+1] <- L.f3.prop %*% time.series.f3.prop[ ,i]
}
matplot(0:length, t(time.series.f3.prop), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="f3 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

time.series.s1.prop <- matrix(0, nrow=nrow(L.s1.prop), ncol=length+1) # same procedure as above but now for the population projection matrix L.s1
time.series.s1.prop[, 1] <- N.0
for (i in 1:length){
  time.series.s1.prop[ , i+1] <- L.s1.prop %*% time.series.s1.prop[ ,i]
}
matplot(0:length, t(time.series.s1.prop), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="s1 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")

time.series.s2.prop <- matrix(0, nrow=nrow(L.s2.prop), ncol=length+1) # same procedure as above but now for the population projection matrix L.s2
time.series.s2.prop[, 1] <- N.0
for (i in 1:length){
  time.series.s2.prop[ , i+1] <- L.s2.prop %*% time.series.s2.prop[ ,i]
}
matplot(0:length, t(time.series.s2.prop), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time", main="s2 increased")
legend("topright", legend = c("age 1", "age 2", "age 3"), lty = 1:3, col = 1, bty = "n")
```
