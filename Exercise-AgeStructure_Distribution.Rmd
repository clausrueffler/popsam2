---
pagetitle: 'Exercise: Structured Populations - State Distribution'
output: 
  html_document:
    toc: FALSE
---
...
# Structured Populations - State Distribution {.tabset}

In the previous exercise, we found that the dynamics of an age-structured population is governed by a single number $\lambda$ that describes how fast the population grows from one time step to the next. More specifically, initially the densities in the different age classes can fluctuate but these fluctuations disappear after some time (transient dynamics) and from then on the population - as a whole as well as the density in each class - grow by the same factor $\lambda$ from one time step to the next. We also found that the value of $\lambda$ does NOT depend on the initial population vector $\mathbf N(0)$ but only on the population projection matrix $\mathsf L$. This value of $\lambda$ is known as the *dominant eigenvalue* of the matrix $\mathsf L$.

In this *R*-exercise you will investigate how the proportions of individuals in the different age classes changes over time. One might for example wonder whether any of the age classes grows or shrinks faster than others and how the proportions of individuals in the different age classes depends on the initial population vector $\mathbf N(0)$?

The proportion (or relative frequency) $p_1(t)$ of individuals in age class 1 at time $t$ is defined as
...
The following code produces a plot of the time series and is taken from the previous tab.
...
matplot(0:length, t(time.series), type="l", lty=1:3, col = 1:3, ylab = "density", xlab = "time") # plots the time.series for each age-class
...
```{r eval=FALSE}
time.series.sum <- colSums(time.series) # vector of length t+1 with the summed densities at each time step
...
time.series.prop <- matrix(c(time.series.prop1, time.series.prop2, time.series.prop3), nrow = 3, byrow = TRUE) # new matrix that contains the proportions in each age-class (rows) for the time series (columns)
