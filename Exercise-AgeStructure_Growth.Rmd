...
```{R}
L[2, 3]  # entry in 2nd row and 3rd column
```
...
```{r eval=FALSE}
N.tot <- colSums(time.series) # produces vector of total densities by summing over the values in each colum of the matrix time.series
...
### 1) By what factor does each age class and the population as a whole grow from one time step to the next?
...
Let us look at the ratio of densites at time step *t* and *(t+1)* for age class *j*. We denote this quantity by $\lambda_j(t)$, i.e.,
...
Let us begin by calculating this quanity for age-class 1 based on a time series of length 40. Note that before you can execute the following command you first have to create a time series of lenght 40.
...
for (i in 1:length){# iterates the multiplication L.N(t) for as many time steps as specified by the parameter "length"
  time.series <- cbind(time.series, L %*% time.series[,i]) # executes multiplication of the matrix L with current population vector and adds the result as a new column to the matrix time.series
}
...
for (i in 1:length){
  Lambda.1 <- cbind(Lambda.1, time.series[1,i+1]/time.series[1,i])
}
...
for (i in 1:length){
  Lambda.2 <- cbind(Lambda.2, time.series[2,i+1]/time.series[2,i]) # here we select densities from the second row of the matrix time.series that contains the densities in age class 1
}
...
We can do the same calculation for the total population size $N_{\mathrm{tot}}$. Before you actually do this calculation think for a second. What do you expect is the outcome and why do you think so?
...
ten.time.series <- matrix(0, nrow = 10*nrow(L), ncol = length+1) # matrix for storing ten time series
...
Note that for later exercises you will have to be able to compute the growht factor $\lambda$ for other matrices.
