...
matplot(0:length, t(time.series), type="l", lty=1:3, col = 1, ylab = "abundance", xlab = "time") # plots the time.series for each age-class, not that we have to transpose the matrix time.series in order to obtain the correct plot
...
Let us begin by calculating this quanity for age-class 1 based on a time series of length 40. Please first change the survival of one-year old individuals back to $s_1=0.3$.
...
for (i in 1:length){
  Lambda.1 <- cbind(Lambda.1, time.series[1,i+1]/time.series[1,i]) # here we divide the population size in class 1 in two consecutive time steps by each other and store the result in Lambda.1
}
