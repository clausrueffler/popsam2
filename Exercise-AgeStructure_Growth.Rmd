---
pagetitle: 'Exercise: Structured Populations - Growth'
output: 
  html_document:
    toc: FALSE
---
...
# Structured Populations - Growth {.tabset}
...
In the following, we revisit some code related to matrix manipulation.
...
<!--
We can create a vector of initital population densities $\mathbf{N}(0)$ by writing
...
Lambda.2 <- c()
for (i in 1:length){
  Lambda.2[i] <- time.series[2,i+1]/time.series[2,i] # we select densities from the second row of the matrix time.series that contains the densities in age class 2
}
plot(1:length, Lambda.2, type="l", ylab = expression(lambda[2](t)=='N'[2](t+1)/'N'[2](t)), xlab = "time")

Lambda.3 <- c()
for (i in 1:length){
  Lambda.3[i] <- time.series[3,i+1]/time.series[3,i]
}
plot(1:length, Lambda.3, type="l", ylab = expression(lambda[3](t)=='N'[3](t+1)/'N'[3](t)), xlab = "time")
```
...
plot(1:length, Lambda.tot, type="l", ylab = expression(lambda[tot](t)=='N'[tot](t+1)/'N'[tot](t)), xlab = "time")
```
